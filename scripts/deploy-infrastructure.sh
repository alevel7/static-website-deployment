...
az network nsg create --resource-group $RESOURCE_GROUP_NAME --name $NETWORK_SECURITY_GROUP --output none

# Allow HTTP (port 80)
az network nsg rule create \
  --resource-group $RESOURCE_GROUP_NAME --nsg-name $NETWORK_SECURITY_GROUP \
...
echo "🌐 Public IP: $(az network public-ip show -g $RESOURCE_GROUP_NAME -n $PUBLIC_IP_NAME --query ipAddress -o tsv)"
